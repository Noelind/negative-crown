...
<x-front::layout>
	...
	<div class="mx-auto px-8 md:ml-[86px] md:pt-28">
		<div class="container mx-auto">
			@if (Route::is('home'))
				<x-front::article.featured :article="$featuredArticle" />
				...
				<x-front::article.simple-grid :articles="$articles->skip(1)" />
			...
			@if (Route::is('all-articles'))
				...
				<x-front::article.simple-grid :articles="$articles" />
				{{ $articles->links() }}
			@endif

			@if (Route::is('author'))
				<h1 class="text-4xl text-black dark:text-white">This is author: {{ $author->name }}</h1>
				<x-front::article.simple-grid :articles="$articles" />
			@endif

			@if (Route::is('category'))
				<h1 class="text-4xl text-black dark:text-white">This is category: {{ $currentCategory->name }}</h1>
				<x-front::article.simple-grid :articles="$articles" />
			@endif
